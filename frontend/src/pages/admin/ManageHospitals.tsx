...
export default function ManageHospitals() {
  ...
  const [hospitals, setHospitals] = useState<(BackendUser & { is_active?: boolean; is_verified?: boolean; date_joined?: string })[]>([]);
  const [loading, setLoading] = useState(true);
  const [statusFilter, setStatusFilter] = useState<'all' | 'verified' | 'pending'>('all');
  const [viewUser, setViewUser] = useState<(typeof hospitals)[number] | null>(null);

  const loadHospitals = async (showLoader = false) => {
    if (showLoader) {
      setLoading(true);
    }
    try {
      const data = await adminApi.users('hospital');
      setHospitals(data);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = hospitals.filter((h) => {
    const matchesSearch = h.full_name.toLowerCase().includes(search.toLowerCase()) || h.mobile.includes(search);
    const matchesStatus = statusFilter === 'all' || (statusFilter === 'verified' ? h.is_verified : !h.is_verified);
    return matchesSearch && matchesStatus;
  });

  const handleExport = () => {
    const headers = ['Hospital', 'Mobile', 'Email', 'Status'];
    const rows = filtered.map((h) => [h.full_name, h.mobile, h.email ?? '', h.is_verified ? 'Verified' : 'Pending']);
  ...
  if (showLoader) return <BrandLoader message="Loading hospitals..." fullScreen={false} />;

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">Manage Hospitals</h1>
          <p className="text-sm text-muted-foreground mt-1">{hospitals.length} partner hospitals</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          ...
          { label: 'Verified', value: hospitals.filter(h => h.is_verified).length, color: 'text-success' },
          { label: 'Active', value: hospitals.filter(h => h.is_active).length, color: 'text-info' },
        ].map((stat) => (
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead className="text-xs uppercase tracking-wider font-semibold text-muted-foreground">Hospital</TableHead>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  <TableRow><TableCell colSpan={5} className="text-center py-10 text-muted-foreground">No hospitals found.</TableCell></TableRow>
                ) : filtered.map((h) => (
                  <TableRow key={h.id} className="hover:bg-muted/30">
                    ...
                    <TableCell className="text-muted-foreground">{h.mobile}</TableCell>
                    <TableCell className="text-muted-foreground">{h.email ?? '—'}</TableCell>
                    <TableCell>
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${h.is_verified ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}>
                        {h.is_verified ? 'Verified' : 'Pending'}
                      </span>
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setViewUser(h)}>View</Button>
                        <Button
                          variant={h.is_verified ? 'outline' : 'default'}
                          size="sm"
                          className="h-8 text-xs"
                          onClick={() => {
                            void toggleVerification(h.id, Boolean(h.is_verified));
                          }}
                        >
                          {h.is_verified ? 'Mark Pending' : 'Verify'}
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>
