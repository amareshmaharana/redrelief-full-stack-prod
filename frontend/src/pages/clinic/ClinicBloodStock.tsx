...
import { clinicApi, mapStock } from "@/lib/backend-api";
...
export default function ClinicBloodStock() {
  ...
  const loadStock = async () => {
    setLoading(true);
    try {
      const data = await clinicApi.stock();
  ...
  const submitForm = async (event: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await clinicApi.updateStock(Number(editingId), {
  ...
  const removeStock = async (id: string) => {
    setDeletingId(id);
    try {
      await clinicApi.deleteStock(Number(id));
      if (editingId === id) {
        resetForm();
      }
