...
import { clinicApi, mapBloodRequest } from "@/lib/backend-api";

const bloodGroups = ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"];

export default function ClinicRequestBlood() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const data = await clinicApi.requests();
      setRequests(data.map((item) => mapBloodRequest(item, "clinic")));
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Submit a blood request for your clinic</p>
      </div>
