...
import { clinicApi, mapStock, mapBloodRequest } from '@/lib/backend-api';
...
export default function ClinicDashboard() {
  ...
  const loadDashboard = async (showLoader = false) => {
    ...
    try {
      const [stockData, reqData] = await Promise.all([clinicApi.stock(), clinicApi.requests()]);
      setStock(stockData.map(mapStock));
      setRequests(reqData.map((r) => mapBloodRequest(r, 'clinic')));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight">Clinic Dashboard</h1>
        ...
        <div className="flex items-center gap-2 rounded-xl bg-amber-500/5 border border-amber-500/10 px-4 py-2">
          ...
          <span className="text-sm font-medium text-amber-600">Clinic Active</span>
        </div>
      </div>
